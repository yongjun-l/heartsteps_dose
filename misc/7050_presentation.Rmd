...
users.clean |>
  ...
  bold_labels() |> 
  as_gt() |> 
  gt::gtsave(filename = "HeartStep Table1.docx")
```
...
#                corstr = "unstructured")
summ <- summary(gee.unadj)$coefficients
rslts.gee.unadj <- round(cbind(summ[,1], summ[,2], summ[,1]-1.96*summ[,4], summ[,1]+1.96*summ[,4]),2)
...
rslts.gee1 <- rbind(rslts.gee.unadj, rslts.gee.adj1)
rslts.gee1
summ.adj
```
...
ylim <- c(min(min(rslts.gee.adj[2:6,]), min(rslts.gee.unadj[2:6,])),
max(max(rslts.gee.adj[2:6,]), max(rslts.gee.unadj[2:6,])))

{
plot(x=(1:5)-0.1, y=rslts.gee.unadj[2:6,1], pch=19, col="brown", ylim = ylim, xlab="Notifications received in a day", ylab = "Change in daily step count", xlim = c(0.8,5.2))
points(1:5-0.1, rslts.gee.unadj[2:6,1], pch=19, col="brown")
points(1:5-0.1, rslts.gee.unadj[2:6,3], pch="-", col="brown")
points(1:5-0.1, rslts.gee.unadj[2:6,4], pch="-", col="brown")
for(i in 1:5){
    lines(c(i-0.1,i-0.1), c(rslts.gee.unadj[i+1,3],
                                    rslts.gee.unadj[i+1,4]), col='brown')
  }
points(1:5+0.1, rslts.gee.unadj[2:6,1], pch=19, col="orange")
points(1:5+0.1, rslts.gee.unadj[2:6,3], pch="-", col="orange")
points(1:5+0.1, rslts.gee.unadj[2:6,4], pch="-", col="orange")
for(i in 1:5){
    lines(c(i+0.1,i+0.1), c(rslts.gee.unadj[i+1,3],
                            rslts.gee.unadj[i+1,4]), col='orange')
}
abline(h=0)
legend("topleft", legend=c("Unadjusted", "Adjusted"),
       col=c("brown", "orange"), pch=19, cex=0.8, text.font=4)
}
...
rslts.gee2 <- cbind(c("Unadjusted", rep("", 5), "Adjusted", rep("", 16)),
                    c("Intercept", "Dose 1", "Dose 2", "Dose 3", "Dose 4", "Dose 5",
                         "Intercept", "Dose 1", "Dose 2", "Dose 3", "Dose 4", "Dose 5",
                         "Age", "Sex", "   Male", "   Female", "Occupation", "   Student", "   Not Student", "Physical Activity", "   Low", "   Moderate", "   High"), rslts.gee1)
colnames(rslts.gee2) <- c("Model", "Variable", "Estimate", "SE", "Lower CI", "Upper CI")
library(kableExtra)
kable(rslts.gee2)
# rslts.gee1 %>%
...
#   as_gt() |> 
#   gt::gtsave(filename = "Aim 1 model coef.docx")

# library(flextable)
# ft1<- flextable(as.data.frame(rslts.gee1) )
# save_as_docx(ft1, path = "Aim1 coef.docx")
```
...
summ.adj.age <- summary(gee.unadj.age)$coefficients
rslts.gee.inter.age <- round(cbind(summ.adj.age[,1], summ.adj.age[,2], summ.adj.age[,1]-1.96*summ.adj.age[,4], summ.adj.age[,1]+1.96*summ.adj.age[,4]), 2)

summ.adj.stu <- summary(gee.unadj.stu)$coefficients
rslts.gee.inter.stu <- round(cbind(summ.adj.stu[,1], summ.adj.stu[,2], summ.adj.stu[,1]-1.96*summ.adj.stu[,4], summ.adj.stu[,1]+1.96*summ.adj.stu[,4]), 2)
```
...
#   column_spec(1, bold = TRUE) %>% # Bold the first column
#   row_spec(0, bold = TRUE)
# ft1<- flextable(as.data.frame(rslts.gee.inter.age1) )
# save_as_docx(ft1, path = "Aim2 age coef.docx")
kable(rslts.gee.inter.age1)
```
```{r}
#rslts.gee.inter.age
meanage <- rslts.gee.inter.age[1:6,]
ageplus <- rslts.gee.inter.age[1:6,] + rslts.gee.inter.age[7:12,]
ylim <- c(min(min(meanage[2:6,]), min(ageplus[2:6,])),
max(max(meanage[2:6,]), max(ageplus[2:6,])))

{
plot(x=(1:5)-0.05, y=meanage[2:6,1], pch=19, col="brown", ylim = ylim, xlab="Notifications received in a day", ylab = "Change in daily step count", xlim = c(0.8,5.2))
points(1:5-0.05, meanage[2:6,1], pch=19, col="brown")
points(1:5-0.05, meanage[2:6,3], pch="-", col="brown")
points(1:5-0.05, meanage[2:6,4], pch="-", col="brown")
for(i in 1:5){
    lines(c(i-0.05,i-0.05), c(meanage[i+1,3],
                                    meanage[i+1,4]), col='brown')
  }
points(1:5+0.05, ageplus[2:6,1], pch=19, col="orange")
points(1:5+0.05, ageplus[2:6,3], pch="-", col="orange")
points(1:5+0.05, ageplus[2:6,4], pch="-", col="orange")
for(i in 1:5){
    lines(c(i+0.05,i+0.05), c(ageplus[i+1,3],
                            ageplus[i+1,4]), col='orange')
}
abline(h=0)
legend("topleft", legend=c("Mean Age", "Mean Age + 10"),
       col=c("brown", "orange"), pch=19, cex=0.8, text.font=4)
}
...
#   column_spec(1, bold = TRUE) %>% # Bold the first column
#   row_spec(0, bold = TRUE)
# ft1<- flextable(as.data.frame(rslts.gee.inter.stu1) )
# save_as_docx(ft1, path = "Aim2 stu coef.docx")
kable(rslts.gee.inter.stu1)
```

```{r}
#rslts.gee.inter.stu
meanage <- rslts.gee.inter.stu[1:6,]
ageplus <- rslts.gee.inter.stu[1:6,]+ rslts.gee.inter.stu[7:12,]
ylim <- c(min(min(meanage[2:6,]), min(ageplus[2:6,])),
max(max(meanage[2:6,]), max(ageplus[2:6,])))
...
abline(h=0)
legend("topleft", legend=c("Student", "Non Student"),
       col=c("brown", "orange"), pch=19, cex=0.8, text.font=4)
}
```
