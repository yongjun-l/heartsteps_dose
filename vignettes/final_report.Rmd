...
for (i in 1:3) {
  df.corr <- simMhealth(m, n[i], time, days, eta, rho, theta1, theta2, beta10, beta11, beta12, p)
  rslt3[i,] <- c(n[i], get.sim.rslts2(m, dfs=df.corr, id="ID", day="DAY", slot="SLOT", p, 
                                      dose=dose, y="Y", trt="A", 
                                      baseline=c("H1", "H2"), timevar=c("S1", "S2"), print_progress=FALSE))
}
rslt3
```
...
```{r sim-4}
m=5; n=5; time=5; days=5; eta=c(1,2,3,4); rho=0;
theta1=c(3,2,1); theta2=0; beta10=1; beta11=0; beta12=0; p=0.5; dose=1


df.corr <- simMhealth(m, n, time, days, eta, rho, theta1, theta2, beta10, beta11, beta12, p)
mHealthDose(df.corr[[1]], id="ID", day="DAY", slot="SLOT", p, 
            dose=dose, y="Y", trt="A",
            baseline=c("H1", "H2"), timevar=c("S1", "S2"), 
            b.prime=NULL, t.prime=NULL,
